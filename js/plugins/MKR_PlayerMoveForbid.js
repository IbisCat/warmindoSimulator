 ...
 * @text Anti-movement switch
 * @desc Specifies the switch number that prohibits the player from moving while it is ON. (Default: 10)
 * @type switch
 * @default 10
 *
 * @param Default_Menu_Flag
 * @text Menu Open/Close Control
 * @desc Set whether to allow the menu to be opened or closed while the player is prevented from moving. (Default: Allowed)
 * @type boolean
 * @on 許可する
 * @off 許可しない
 * @default true
 *
 * @param Enter Flag
 * @text Decision key control
 * @desc Set whether to allow actions using the Enter key/touch operation while the player's movement is prohibited. (Default: Allow)
 * @type boolean
 * @on 許可する
 * @off 許可しない
 * @default true
